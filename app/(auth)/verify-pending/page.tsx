...
function VerifyPendingContent() {
    ...
    const checkStatus = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                `${authUrl("/verification-status")}?email=${encodeURIComponent(email)}`
            );
    ...
    async function handleResend() {
        ...
        try {
            const res = await fetch(authUrl("/resend-verification"), {
                method: "POST",
    ...
    if (!email) {
        return (
            <AuthFormWrapper title="Missing email" description="No email address was provided.">
                <Link
                    href="/signUp"
                    className="mt-2 inline-flex h-10 w-full items-center justify-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground shadow-sm hover:bg-primary/90"
                >
                    Go to Sign Up
                </Link>
            </AuthFormWrapper>
