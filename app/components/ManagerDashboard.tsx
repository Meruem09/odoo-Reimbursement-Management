...
type Category = "Travel" | "Meals" | "Software" | "Equipment" | "Training" | "Other" | string;
...
function ApprovalsModal({ expense, onClose, onAction }: { expense: Expense | null; onClose: () => void; onAction: (id: string, action: ExpenseStatus, comment: string) => void; }) {
    ...
    const handleActionLocal = async (decision: ExpenseStatus) => {
        setIsPending(true);
        await onAction(expense.id, decision, comment);
        setIsPending(false);
    };
...
function DashboardView({ expenses, onAction }: { expenses: Expense[], onAction: (id: string, act: ExpenseStatus, c: string) => void }) {
    ...
    const filtered = expenses.filter(e => {
        ...
        return matchTab && (e.employee.toLowerCase().includes(q) || e.category.toLowerCase().includes(q) || e.desc.toLowerCase().includes(q));
    });

    const handleModalAction = async (id: string, action: ExpenseStatus, comment: string) => {
        await onAction(id, action, comment);
        setModal(null);
    };
    ...
    return (
        <div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                <div className="flex items-center justify-between px-4 border-b border-slate-200 w-full overflow-x-auto">
                    ...
                    <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." className="hidden md:block w-48 px-3 py-1.5 text-[11px] border border-slate-200 rounded-md bg-slate-50 text-slate-900 outline-none focus:border-blue-500 ml-4 shrink-0" />
                </div>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filtered.length === 0 ? (
                                ...
                                <tr key={e.id} className={i % 2 === 0 ? "bg-white hover:bg-slate-50/50 transition-colors" : "bg-slate-50/30 hover:bg-slate-50/80 transition-colors"}>
                                    <td className="p-3 px-4 w-[140px]">
                                        <div className="font-bold text-[11px] text-slate-900 truncate">{e.employee}</div>
                                        <div className="text-[9px] text-slate-500 truncate">{e.dept}</div>
                                    </td>
                                    <td className="p-3 px-4 w-[110px]">
                                        <span className="text-[10px] text-slate-500 truncate block">{catIcon[e.category] || "▪"} {e.category}</span>
                                    </td>
                                    <td className="p-3 px-4">
                                        <div className="text-[11px] text-slate-500 line-clamp-2" title={e.desc}>{e.desc}</div>
                                    </td>
                                    <td className="p-3 px-4 w-[85px]">
                                        <span className="text-[10px] text-slate-500 whitespace-nowrap">{e.date}</span>
                                    </td>
                                    <td className="p-3 px-4 w-[85px]">
                                        <span className="font-extrabold text-[12px] text-slate-900 whitespace-nowrap">{e.display}</span>
                                    </td>
                                    <td className="p-3 px-4 w-[100px]"><Badge status={e.status} /></td>
                                    <td className="p-3 px-4 w-[85px]">
                                        {e.status === "pending" ? (
                                            <button onClick={() => setModal(e)} className="px-3 py-1.5 bg-blue-600 text-white rounded text-[10px] font-bold hover:bg-blue-700 transition-colors shadow-sm whitespace-nowrap">Review</button>
                                        ) : (
                                            <button onClick={() => setModal(e)} className="px-3 py-1.5 bg-transparent border border-slate-200 text-slate-500 rounded text-[10px] font-semibold hover:bg-slate-50 transition-colors whitespace-nowrap">View</button>
                                        )}
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="px-4 py-3 border-t border-slate-200 flex justify-between items-center bg-slate-50">
                    <span className="text-[11px] text-slate-500">Showing {filtered.length} of {expenses.length}</span>
                    <div className="flex gap-1.5">
                        <button className="px-2.5 py-1 border border-slate-200 bg-white rounded flex items-center justify-center text-[10px] text-slate-500 hover:bg-slate-50 transition-colors">←</button>
                        <button className="px-2.5 py-1 border border-slate-200 bg-white rounded flex items-center justify-center text-[10px] text-slate-500 hover:bg-slate-50 transition-colors">→</button>
                    </div>
                </div>
            </div>
        ...
        return (
            <div>
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-8 text-center shadow-sm">
                    <p className="text-sm text-slate-500 font-medium">You currently have no direct reports.</p>
                </div>
            </div>
        );
    }
...
function SettingsView({ user }: { user: any }) {
    ...
    const inputClass = "w-full p-2 border border-slate-200 rounded-md text-xs font-sans text-slate-900 bg-slate-50 outline-none focus:border-blue-500 focus:bg-white transition-colors h-9";
                    ...
                    <div>
                        ...
                        <div className="bg-white border border-red-200 rounded-xl overflow-hidden shadow-sm">
                            <div className="p-4 flex justify-between items-center">
                                ...
                                <button onClick={() => window.location.reload()} className="px-3.5 py-1.5 border border-red-600 rounded text-[11px] font-bold text-red-600 hover:bg-red-50 transition-colors bg-white">
                                    Refresh
                                </button>
                            </div>
                        </div>
...
export default function ManagerDashboard() {
    ...
    // Fetch live data from the unified API endpoint
    useEffect(() => {
    ...
    const handleAction = async (id: string, decision: ExpenseStatus, comment: string) => {
        ...
        // Network request to process approval action
        const reqDecision = decision === "approved" ? "approved" : "rejected";
        try {
            ...
            if (!res.ok) {
                ...
                console.error("Failed setting approval.");
            }
        } catch (e) {
        ...
        return (
            <div className="w-full h-full flex items-center justify-center p-12">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="text-xs font-semibold text-slate-500 uppercase tracking-widest">Loading Dashboard...</div>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="w-full h-full flex flex-col pt-8 px-6 lg:px-10 max-w-7xl mx-auto bg-[#F1F5F9] min-h-[calc(100vh-64px)] overflow-x-hidden pb-12">
            {/* Top Navigation */}
            <div className="flex overflow-x-auto gap-2 mb-8 border-b border-slate-200 pb-px shrink-0">
                {[
                    ...
                    <button 
                        ...
                        className={`px-4 py-2.5 text-sm font-bold rounded-t-lg border-b-[3px] transition-colors whitespace-nowrap ${activeTab === item.key ? "text-blue-600 border-blue-600 bg-blue-50/50" : "text-slate-500 border-transparent hover:text-slate-800 hover:bg-slate-100/50"}`}
                    >
                        {item.label}
                    </button>
                ))}
            ...
            {/* Main Content Area */}
            <div className="flex-1 animate-in fade-in duration-300 relative w-full">
